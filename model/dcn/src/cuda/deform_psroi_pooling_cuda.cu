    ...
    const int height)
{
  ...
  T value = (static_cast<T>(1) - dist_x) * (static_cast<T>(1) - dist_y) * value11 +
            (static_cast<T>(1) - dist_x) * dist_y * value12 +
            dist_x * (static_cast<T>(1) - dist_y) * value21 +
    ...
    T *top_count)
{
  CUDA_KERNEL_LOOP(index, count)
  {
    ...
    T roi_start_w = static_cast<T>(round(offset_bottom_rois[1])) * spatial_scale - static_cast<T>(0.5);
    T roi_start_h = static_cast<T>(round(offset_bottom_rois[2])) * spatial_scale - static_cast<T>(0.5);
    T roi_end_w = static_cast<T>(round(offset_bottom_rois[3]) + 1.) * spatial_scale - static_cast<T>(0.5);
    T roi_end_h = static_cast<T>(round(offset_bottom_rois[4]) + 1.) * spatial_scale - static_cast<T>(0.5);

    T roi_width = max(roi_end_w - roi_start_w, static_cast<T>(0.1));
    ...
    T trans_y = no_trans ? static_cast<T>(0)
    ...
    for (int ih = 0; ih < sample_per_part; ih++)
    {
      for (int iw = 0; iw < sample_per_part; iw++)
      {
        ...
        if (w < static_cast<T>(-0.5) || w > static_cast<T>(width - 0.5) ||
            h < static_cast<T>(-0.5) || h > static_cast<T>(height - 0.5))
        {
          continue;
        }

        w = min(max(w, static_cast<T>(0.0)), static_cast<T>(width - 1.0));
        h = min(max(h, static_cast<T>(0.0)), static_cast<T>(height - 1.0));
    ...
    top_data[index] = sample_count == 0 ? static_cast<T>(0) : sum / sample_count;
    top_count[index] = sample_count;
  }
}
  ...
  CUDA_KERNEL_LOOP(index, count)
  {
    ...
    for (int ih = 0; ih < sample_per_part; ih++)
    {
      for (int iw = 0; iw < sample_per_part; iw++)
      {
        ...
        T q00 = (static_cast<T>(1) - dist_x) * (static_cast<T>(1) - dist_y);
        T q01 = (static_cast<T>(1) - dist_x) * dist_y;
        T q10 = dist_x * (static_cast<T>(1) - dist_y);
        T q11 = dist_x * dist_y;
        ...
        T diff_x = (U11 * dist_y + U10 * (static_cast<T>(1) - dist_y) -
                    U01 * dist_y - U00 * (static_cast<T>(1) - dist_y)) *
                   trans_std * diff_val;
        diff_x *= roi_width;

        T diff_y = (U11 * dist_x + U01 * (static_cast<T>(1) - dist_x) -
                    U10 * dist_x - U00 * (static_cast<T>(1) - dist_x)) *
                   trans_std * diff_val;
        diff_y *= roi_height;
                                  ...
                                  const float trans_std)
{
  ...
  TORCH_CHECK(bbox.is_cuda(), "rois must be a CUDA tensor");
  TORCH_CHECK(trans.is_cuda(), "trans must be a CUDA tensor");
  ...
  TORCH_CHECK(channels == output_dim, "input channels and output channels must equal");
  ...
  long out_size = static_cast<long>(num_bbox) * output_dim * pooled_height * pooled_width;
  ...
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "deform_psroi_pooling_cuda_forward", [&] {
    DeformablePSROIPoolForwardKernel<scalar_t><<<grid, block, 0, stream>>>(
        ...
        no_trans,
        static_cast<scalar_t>(trans_std),
        sample_per_part,
        output_dim,
        group_size,
        part_size,
...
deform_psroi_pooling_cuda_backward(const at::Tensor &out_grad,
                                   ...
                                   const float trans_std)
{
  TORCH_CHECK(out_grad.is_cuda(), "out_grad must be a CUDA tensor");
  TORCH_CHECK(input.is_cuda(), "input must be a CUDA tensor");
  TORCH_CHECK(bbox.is_cuda(), "bbox must be a CUDA tensor");
  TORCH_CHECK(trans.is_cuda(), "trans must be a CUDA tensor");
  TORCH_CHECK(top_count.is_cuda(), "top_count must be a CUDA tensor");
  ...
  AT_DISPATCH_FLOATING_TYPES(out_grad.scalar_type(), "deform_psroi_pooling_cuda_backward", [&] {
