...
at::Tensor deform_conv_cuda_forward(
...
std::vector<at::Tensor> deform_conv_cuda_backward(
    const at::Tensor &input,
    const at::Tensor &weight,
    const at::Tensor &bias,
    const at::Tensor &offset,
    ...
    for (int n = 0; n < batch / im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "deform_conv_backward_cuda", [&]
            {
                ...
                // gradient w.r.t. weight, dWeight should accumulate across batch and group
                deformable_im2col_cuda(
                    at::cuda::getCurrentCUDAStream(),
                    input.data_ptr<scalar_t>() + n * im2col_step_ * per_input_size,
                    offset.data_ptr<scalar_t>() + n * im2col_step_ * per_offset_size,
